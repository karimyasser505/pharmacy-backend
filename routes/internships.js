...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const internship = await db.getSQL('SELECT * FROM internships WHERE id = $1', [id]);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const query = `
      UPDATE internships 
      SET title = $1, type = $2, duration = $3, deadline = $4, description = $5, 
          requirements = $6, benefits = $7, status = $8, updated_at = CURRENT_TIMESTAMP
      WHERE id = $9
    `;
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const result = await db.runSQL('DELETE FROM internships WHERE id = $1', [id]);
